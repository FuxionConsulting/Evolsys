...
from ...tools import evolsys_cne_query
...
class ResPartner(models.Model):
    ...
    def create(self, vals_list):
        ...
        calling the function get_default_name_by_vat from evolsys_cne_query before create the partner
        ...
        for vals in vals_list:
            if vals.get("vat") and not vals.get("name", False):
                ...
                if prefix_vat == "V" and not name and prefix_vat in ["V", "E"]:
                    name, flag = evolsys_cne_query.get_default_name_by_vat(
                        self, prefix_vat, vat
                    )
                    if not flag:
                        continue
    ...
    def _onchange_(self):
        """This function assign the name of the person by the vat number and the prefix of the vat number
        calling the function get_default_name_by_vat from evolsys_cne_query
        ...
        if self.vat and not self.name and self.prefix_vat in ["V", "E"]:
            self._check_vat()
            name, flag = evolsys_cne_query.get_default_name_by_vat(self, self.prefix_vat, self.vat)
            if not flag:
                return
            for record in self:
                record.name = name
