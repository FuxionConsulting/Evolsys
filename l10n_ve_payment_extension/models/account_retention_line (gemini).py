from odoo import api, fields, models, _ # Añadida la importación de _ para traducción
...
class AccountRetentionLine(models.Model):
    ...
    # Asegúrate de que 'account.retention' tiene un campo 'state' si lo usas en el related
    retention_id = fields.Many2one(
        "account.retention",
        string="Retention",
        required=True, # Sugerencia: hacer este campo requerido para asegurar la consistencia
        ondelete="cascade"
    )
    ...
    foreign_retention_amount = fields.Float(
        digits="Tasa", compute="_compute_retention_amount", store=True, readonly=False # Se eliminó la definición redundante de foreign_retention_amount
    )
    ...
    # Este campo 'payment_id' es el inverse_name del One2many en account.payment.
    # Es crucial que se llame 'payment_id' para que la relación One2many funcione correctamente.
    payment_id = fields.Many2one(
        "account.payment",
        string="Payment", # Cambiado 'Payment' a 'Pago' para consistencia en español
        required=True, # Sugerencia: hacer este campo requerido para asegurar la consistencia
        ondelete="cascade",
        index=True
    )
    ...
    # foreign_retention_amount ya está definido arriba con compute
    foreign_currency_rate = fields.Float(string="Rate")

    @api.depends("retention_id.type_retention", "move_id")
    def _compute_name(self):
        for record in self:
            if record.name and record.name != 'Nueva Línea de Retención': # Evitar sobrescribir si ya tiene un nombre útil
                continue
            names = {
            ...
            type_retention = "islr"
            if record.retention_id and record.retention_id.type_retention: # Asegurar que retention_id existe
                type_retention = record.retention_id.type_retention
            elif record.move_id:
                # Estas líneas asumen que existen campos one2many en account.move apuntando a esta línea
                # Por ejemplo: move_id.retention_iva_line_ids y move_id.retention_municipal_line_ids
    ...
    # CRÍTICO: REVISAR ESTE MÉTODO UNLINK. ¡Casi seguro que esto no es lo deseado!
    # Eliminar 'record.payment_id.unlink()' aquí, a menos que estés absolutamente seguro
    # de que quieres eliminar el pago completo cada vez que una línea de retención se elimina.
    # Lo más común es que la línea se elimine en cascada cuando el pago se elimina,
    # o que solo el pago se elimine si ya no tiene líneas asociadas.
    def unlink(self):
        for record in self:
            # Comentado para evitar eliminación no deseada del pago padre
            # record.payment_id.unlink()
            pass # No hacer nada o implementar una lógica más segura
    ...
    def _compute_amounts(self):
        ...
        if not base_currency_is_vef:
            for line in self:
                # Asegurarse de que foreign_currency_rate no sea cero para evitar ZeroDivisionError
                if line.foreign_currency_rate:
                    line.invoice_amount = line.foreign_invoice_amount * (
                        1 / line.foreign_currency_rate
                    )
                #else: # Puedes añadir un manejo para cuando la tasa es cero
    ...
    def _compute_retention_amount(self):
        ...
        islr_supplier_retention_lines = self.filtered(
            ...
            or (l.retention_id and l.retention_id.type_retention == "islr" and l.retention_id.type == "in_invoice") # Añadido 'l.retention_id and' para evitar errores si retention_id es False
        )
        for record in islr_supplier_retention_lines:
            ...
            # Asegurarse de que los porcentajes no sean None o causen division por cero
            perc_tax_base = record.related_percentage_tax_base / 100.0 if record.related_percentage_tax_base else 0.0
            perc_fees = record.related_percentage_fees / 100.0 if record.related_percentage_fees else 0.0
            amount_sub_fees = record.related_amount_subtract_fees if record.related_amount_subtract_fees else 0.0
            ...
            if not base_currency_is_vef:
                record.retention_amount = (
                    ...
                    * perc_tax_base
                    * perc_fees
                ) - (amount_sub_fees / foreign_rate) if foreign_rate else amount_sub_fees
            else:
                record.retention_amount = (
                    record.invoice_amount
                    * perc_tax_base
    ...
    def onchange_economic_activity_id(self):
        ...
        municipal_retention_lines_with_economic_activity_and_invoice = self.filtered(
            lambda l: (not l.retention_id or (l.retention_id and l.retention_id.type_retention == "municipal")) # Añadido 'l.retention_id and'
        ...
        for record in municipal_retention_lines_with_economic_activity_and_invoice:
            ...
            # Asegurarse de que aliquot no sea None o cause problemas
            aliquot_val = record.aliquot if record.aliquot else 0.0
            record.retention_amount = record.invoice_amount * aliquot_val / 100
            record.foreign_retention_amount = record.foreign_invoice_amount * aliquot_val / 100
    ...
    def onchange_municipal_invoice_amount(self):
        ...
        for record in self.filtered(
            ...
            or (l.retention_id and l.retention_id.type_retention == "municipal") # Añadido 'l.retention_id and'
        ):
            # Asegurarse de que aliquot no sea None o cause problemas
            aliquot_val = record.aliquot if record.aliquot else 0.0
            record.retention_amount = record.invoice_amount * aliquot_val / 100
    ...
    def onchange_retention_amount(self):
        ...
        for line in self.filtered(
            lambda l: not l.retention_id or (l.retention_id and l.retention_id.type == "out_invoice") # Añadido 'l.retention_id and'
        ):
            self.env.context = self.with_context(noonchange=True).env.context
            if not line.retention_id or (line.retention_id and line.retention_id.type_retention in ("islr", "municipal")): # Añadido 'line.retention_id and'
                # Asegurarse de que foreign_inverse_rate no sea cero para evitar ZeroDivisionError
            ...
            self.env.context = self.with_context(noonchange=True).env.context
            if line.move_id and line.move_id.foreign_inverse_rate: # Asegurarse de que foreign_inverse_rate no sea cero
                line.update(
                    {
    ...
    def onchange_foreign_retention_amount(self):
        ...
        for line in self.filtered(
            ...
            if not line.retention_id or (line.retention_id and line.retention_id.type_retention in ("islr", "municipal")): # Añadido 'line.retention_id and'
                # Asegurarse de que foreign_rate no sea cero para evitar ZeroDivisionError
            ...
            self.env.context = self.with_context(noonchange=True).env.context
            if line.move_id and line.move_id.foreign_rate: # Asegurarse de que foreign_rate no sea cero
                line.update(
                    {
    ...
    def _constraint_amounts(self):
        for record in self:
            # Simplificación de la validación para 0, permite que cualquier campo sea 0 siempre que no todos lo sean
            # Si quieres que NINGUNO sea 0, la validación original era correcta.
            # Aquí se ajusta para permitir 0 si los demás no lo son (por ejemplo, si no aplica un monto específico)
            if all(
                (
            ...
            is_client_retention = record.retention_id and record.retention_id.type == "out_invoice" # Añadido 'record.retention_id and'
            if (
                is_vef_the_base_currency
                and is_client_retention
                and record.move_id and record.retention_amount > record.move_id.amount_residual # Añadido 'record.move_id and'
            ):
    ...
    def get_invoice_paid_amount_not_related_with_retentions(self):
        ...
        for line in self.filtered(
            lambda l: l.retention_id and l.retention_id.type_retention == "islr" # Añadido 'l.retention_id and'
        ):
